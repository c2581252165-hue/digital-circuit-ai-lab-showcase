...
const truthTableHeadB = document.getElementById("truthTableHeadB");
...
const gateMeta = {
    AND: {
        label: "与门",
        explain: "与门要求两个输入都为 1 时，输出才为 1。"
    },
    OR: {
        label: "或门",
        explain: "或门只要有一个输入为 1，输出就会变成 1。"
    },
    NOT: {
        label: "非门",
        explain: "非门只看输入 A，并把它翻转成相反的输出。A 为 0 时输出 1，A 为 1 时输出 0。"
    },
    XOR: {
        label: "异或门",
        explain: "异或门在两个输入不相同的时候，输出才为 1。"
    }
};
...
function isUnaryGate(gate = state.gate) {
    return gate === "NOT";
}
...
function renderTruthTable() {
    ...
    const rows = isUnaryGate()
        ? [[0, "-"], [1, "-"]]
        : [[0, 0], [0, 1], [1, 0], [1, 1]];

    rows.forEach(([a, b]) => {
        const output = calculateOutput(a, Number(b === "-" ? 0 : b), state.gate);
        const row = document.createElement("tr");
        row.id = isUnaryGate() ? `row-not-${a}` : `row-${a}${b}`;
        row.innerHTML = `<td>${a}</td><td>${b}</td><td>${output}</td>`;
...
function highlightTruthRow() {
    ...
    const activeRow = document.getElementById(isUnaryGate() ? `row-not-${state.A}` : `row-${state.A}${state.B}`);
    if (activeRow) {
...
function legacySetStreamingBubbleText(text) {
    bubbleText.textContent = text;
    bubbleText.classList.add("is-streaming");
    bubbleText.scrollTop = bubbleText.scrollHeight;
}

function legacyStartSpeechSyncedBubbleText(text, baseDelay = 118) {
    stopStreamingText();

    const characters = Array.from(text);
    let index = 0;
    legacySetStreamingBubbleText("");
    ...
    function next() {
        ...
        const character = characters[index];
        index += 1;
        legacySetStreamingBubbleText(characters.slice(0, index).join(""));
...
function legacySyncSpeechBubbleToBoundary(text, event) {
    window.clearTimeout(speechTextSyncTimer);
    speechTextSyncTimer = null;

    const characters = Array.from(text);
    const nextIndex = Math.min(characters.length, Math.max(1, Number(event?.charIndex ?? 0) + 1));
    legacySetStreamingBubbleText(characters.slice(0, nextIndex).join(""));
}
...
function getCurrentTruthRowId() {
    return isUnaryGate() ? `row-not-${state.A}` : `row-${state.A}${state.B}`;
}

function updateSecondaryInputAvailability() {
    const disabled = isUnaryGate();

    if (truthTableHeadB) {
        truthTableHeadB.textContent = disabled ? "输入 B（未使用）" : "输入 B";
    }

    if (btnB) {
        btnB.disabled = disabled;
        btnB.classList.toggle("is-disabled", disabled);
        btnB.title = disabled ? "当前切换到非门，B 输入不会参与运算。" : "";
    }
}

function updateCircuit(speakText = "") {
    const result = calculateOutput(state.A, state.B, state.gate);
    const unaryGate = isUnaryGate();

    valueA.textContent = state.A;
    valueB.textContent = unaryGate ? "-" : state.B;
    wireValueA.textContent = state.A;
    wireValueB.textContent = unaryGate ? "-" : state.B;
    outputValue.textContent = result;
    gateVisual.textContent = state.gate;
    gateLabel.textContent = `当前逻辑门：${gateMeta[state.gate].label}`;
    updateSecondaryInputAvailability();

    updateSwitchStyle(btnA, state.A);
    updateSwitchStyle(btnB, state.B);

    wireA.parentElement.classList.toggle("active", state.A === 1);
    wireB.parentElement.classList.toggle("active", !unaryGate && state.B === 1);
    wireOut.parentElement.classList.toggle("active", result === 1);
    ledOutput.classList.toggle("on", result === 1);

    renderTruthTable();
    highlightTruthRow();
    interactionMetrics.lastExplanation = speakText || buildResultExplain();
...
function buildResultExplain() {
    const result = calculateOutput(state.A, state.B, state.gate);
    if (isUnaryGate()) {
        return `当前输入 A 为 ${state.A}。在${gateMeta[state.gate].label}条件下，系统只看 A，所以输出 Y 为 ${result}。`;
    }

    return `当前输入 A 为 ${state.A}，输入 B 为 ${state.B}。在${gateMeta[state.gate].label}条件下，输出 Y 为 ${result}。`;
}

function describeInputChange(inputKey) {
    const gate = gateMeta[state.gate];
    const text = `${inputKey === "A" ? "输入 A" : "输入 B"} 已切换为 ${state[inputKey]}。${gate.explain}${buildResultExplain()}`;
    stepHint.textContent = isUnaryGate()
        ? `你刚刚修改了输入 ${inputKey}。当前是非门模式，只需要关注 A 如何被翻转成输出。`
        : `你刚刚修改了输入 ${inputKey}。继续切换另一个输入，或改一下逻辑门，比较输出差异。`;
    focusState.textContent = `输入 ${inputKey} 已更新`;
    interactionMetrics.lastAction = `切换输入 ${inputKey}`;
    interactionMetrics.actionCount += 1;
    updateCircuit(text);
}

function describeGateChange(gate) {
    const gateInfo = gateMeta[gate];
    const text = `你现在切换到了${gateInfo.label}。${gateInfo.explain}${buildResultExplain()}`;
    stepHint.textContent = gate === "NOT"
        ? "当前已经切换到非门。你只需要切换 A，就能观察输入翻转后的输出变化。"
        : `当前已经切换到${gateInfo.label}。现在可以多试几组输入，观察真值表高亮的变化。`;
    focusState.textContent = `${gateInfo.label}讲解`;
    interactionMetrics.lastAction = `切换逻辑门到 ${gateInfo.label}`;
    interactionMetrics.actionCount += 1;
    updateCircuit(text);
}

function toggleInput(inputKey) {
    if (inputKey === "B" && isUnaryGate()) {
        return;
    }
...
function setGate(gate) {
    ...
    describeGateChange(gate);
}

function playIntro() {
    introText = "欢迎进入实验一，基础逻辑门仿真。你可以切换输入 A 和输入 B，再尝试与门、或门、非门和异或门。我会在你每次操作时解释输出为什么会变化。";
    stepHint.textContent = "实验介绍已播放。先点击输入开关，再观察输出灯和真值表。";
    focusState.textContent = "实验介绍";
    interactionMetrics.lastAction = "播放实验介绍";
...
function stopDemoPlayback() {
    demoRunId += 1;
    clearDemoTimers();
    interactionMetrics.demoRunning = false;
}
...
function applyDemoState(config, narration) {
    if (config.gate) {
        ...
        gateButtons.forEach((button) => {
            button.classList.toggle("active", button.dataset.gate === config.gate);
        });
    }
...
async function startDemoPlayback() {
    ...
    interactionMetrics.demoRunning = true;
    interactionMetrics.demoRuns += 1;
    interactionMetrics.lastAction = "开始自动演示";

    focusState.textContent = "自动演示";
    stepHint.textContent = "小芯正在自动演示基础逻辑门实验，请观察输入、逻辑门类型和输出变化。";
    ...
    const steps = [
        {
            ...
            A: 0,
            B: 0,
            narration: "第一步，我们先看与门，输入 A 和输入 B 都是 0，所以输出 Y 也保持 0。"
        },
        {
            gate: "AND",
            A: 1,
            B: 0,
            narration: "现在把 A 调成 1，而 B 仍然是 0。与门要求两个输入同时为 1，所以输出依然是 0。"
        },
        {
            gate: "AND",
            A: 1,
            B: 1,
            narration: "接着把 B 也调成 1。这时候与门的两个输入都为 1，输出 Y 就亮起变成了 1。"
        },
        {
            gate: "OR",
            A: 1,
            B: 0,
            narration: "下面切换到或门。或门只要有一个输入为 1，输出就会变成 1，所以现在 Y 仍然是 1。"
        },
        {
            gate: "NOT",
            A: 0,
            B: 0,
            narration: "再看非门。非门只需要一个输入 A，当 A 为 0 时，它会把输出翻转成 1，所以现在 Y 亮起。"
        },
        {
            gate: "XOR",
            A: 1,
            B: 0,
            narration: "最后看异或门。异或门在两个输入不同的时候输出 1，所以 A 为 1、B 为 0 时，输出也是 1。"
        },
        {
            gate: "XOR",
            A: 1,
            B: 1,
            narration: "当两个输入都变成 1 时，异或门反而会输出 0。这样三种基础逻辑门的规律就完整演示了一遍。"
        }
    ];
    ...
    try {
        ...
        stepHint.textContent = "基础逻辑门整套演示已完成。你现在可以自己切换输入和逻辑门，继续观察输出变化。";
        interactionMetrics.demoRunning = false;
        interactionMetrics.lastAction = "自动演示完成";
        speak("整个基础逻辑门实验已经演示完成。现在你可以自己动手继续操作。");
    } catch (_error) {
        // ignore cancellation
        interactionMetrics.demoRunning = false;
    }
...
window.__experimentVoiceHooks = {
    ...
    getExperimentState() {
        ...
        return {
            ...
            B: state.B,
            output: result,
            gateLabel: gateMeta[state.gate].label,
            unaryGate: isUnaryGate(),
            availableGates: Object.keys(gateMeta),
            activeTruthRow: getCurrentTruthRowId(),
            ...
            lastAction: interactionMetrics.lastAction,
            actionCount: interactionMetrics.actionCount,
            demoRunning: interactionMetrics.demoRunning,
            demoRuns: interactionMetrics.demoRuns,
            explanation: buildResultExplain()
        };
    },
    interrupt() {
        stopDemoPlayback();
